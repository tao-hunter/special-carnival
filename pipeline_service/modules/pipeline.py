...
class GenerationPipeline:
    ...
    async def generate_gs(self, request: GenerateRequest) -> GenerateResponse:
        ...
        # 1. Edit the image using Qwen Edit (preprocessing + background removal)
        # This now performs both preprocessing and background removal in two passes
        ...
        if self.settings.save_generated_files:
            save_files(trellis_result, image_edited, image_edited)
        ...
        response = GenerateResponse(
            ...
            image_without_background_file_base64=image_edited_base64 if self.settings.send_generated_files else None,
        )
        return response
